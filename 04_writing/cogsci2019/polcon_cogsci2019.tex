...
{\centering \includegraphics{figs/fig_example_placement-1} 

}

\caption[Example story]{Example story.}\label{fig:fig_example_placement}
\end{figure*}
\end{CodeChunk}

\subsubsection{Stimuli and design}\label{stimuli-and-design}

We designed a picture book (see Figure~\ref{fig:fig_example_placement})
with twelve stories in which a protagonist is approached by two
speakers, one of whom makes a request by producing an utterance with a
politeness marker (e.g., ``Please pour me more water''), and the other
produces an utterance without (``Pour me more water''). There were three
types of politeness marker that could be used: ``please'' (as in
``Please pour me more water''), ``can you'' (``Can you pour me more
...
(Figure~\ref{fig:fig_results_placement}, top row). A mixed-effects
...
0.2, \(p =\) .026). The regression model also revealed that children
...
\emph{polite} and \emph{rude} questions (\(\beta\) = 0.8, \(p =\) .002),
whereas social implication questions (\emph{play partner} and
\emph{compliance}) were overall more difficult compared to speaker
attribute questions (\emph{polite}, \emph{rude}, \emph{nice}, and
\emph{mean}; \(\beta\) = -0.33, \(p =\) .006).
...
presence/absence of a politeness marker (e.g., the speakers were
instructed to record ``Please pour me more water'' with a ``kind
voice''" and ``pour me more water'' with an ``angry voice'').
...
(Figure~\ref{fig:fig_results_placement}, bottom row) compared to Exp. 1.
...
We conducted a third, preregistered experiment to see whether children
...
{\centering \includegraphics{figs/fig_e3results_placement-1} 

}

\caption[Experiment 3 results]{Experiment 3 results. Conventions are identical to Figure 2.}\label{fig:fig_e3results_placement}
\end{figure*}
\end{CodeChunk}
...
2-year-olds. (Fig. ~\ref{fig:fig_e3results_placement}).
...
showed improvement with age (\(\beta\) = 0.19, \(p =\) .033) as well as
better performance for ``can you please'' than ``please'' and ``can
you'' together (\(\beta\) = 0.42, \(p =\) .002), consistent with
Experiment 2 results. Performance for ``please'' was also better than
for ``can you please'' and ``can you'' together (\(\beta\) = .3, \(p =\)
.027), which may be surprising given that we previously did not see the
...
\emph{rude} overall (\(\beta\) = -0.19, \(p =\) .04), but especially
given ``please'' (\(\beta\) = .42, \(p =\) .002). Finally, children
showed a greater improvement with age for ``can you please'' compared to
``please'' and ``can you'' together (\(\beta\) = 0.38, \(p =\) .004).
...
.011), confirming results from each individual experiment. Additionally,
the model showed that children became better at judging the politeness
marker ``can you please'' with age (\(\beta\) = 0.73, \(p =\) .005), and
that children answered \emph{polite} questions better than \emph{rude}
questions about the marker ``please'' (\(\beta\) = 0.26, \(p =\) .006)
