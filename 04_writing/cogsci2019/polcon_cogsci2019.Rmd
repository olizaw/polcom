...
A mixed-effects logistic regression predicting accuracy based on age, question type and marker type^[for each experiment, we use this model structure: `accuracy ~ age x question type x marker type + (1 | item)`, where age is centered and scaled. All categorical variables were deviation coded, with specified contrasts of interest for the question type. Significance was calculated using the standard normal approximation to the $t$ distribution [@barr2013].] confirmed there was an improvement with age ($\beta$ = `r v1_lmer_coeff$estimate[2]`, $p =$ `r v1_lmer_coeff$p.val.clean[2]`) and questions were answered more accurately for "can you please" compared to "please" ($\beta$ = `r v1_lmer_coeff$estimate[7]`, $p =$ `r v1_lmer_coeff$p.val.clean[7]`).
Additionally, some 
